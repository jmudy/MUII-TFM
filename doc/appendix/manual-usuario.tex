...
Anaconda se trata de la \textit{suite} más compleja para la Ciencia de Datos en R y Python, lenguajes de programación que actualmente son líderes en Machine Learning, Inteligencia Artificial y Big Data. Esta \textit{suite} gratuita y multiplataforma dispone de las \acrshort{ide}'s y librerías adecuadas para su manejo. Con ellos se evita tener que instalar manualmente un \acrshort{ide} y el lenguaje Python con sus correspondientes librerías que en ocasiones puede ser una operación tediosa y compleja.
...
Con la finalidad de evitar tediosa y larga instalación de \acrshort{cuda} y \acrshort{cudnn} y tener únicamente instaladas las librerías necesarias para que funcione el código se va a instalar un entorno virtual en Anaconda.

Desde la misma carpeta del repositorio de Github \ref{lst:descarga-repo} que se ha clonado hay dos ficheros con extensión .yml, donde viene indicadas la versión de Python que se va a necesitar, la versión de \acrshort{cuda} y \acrshort{cudnn} así como las librerías y dependencias necesarias. Para utilizar \acrshort{cuda} es necesario disponer de una \acrshort{gpu} de NVIDIA. Es muy importante verificar la capacidad de cálculo de la tarjeta gráfica NVIDIA que se emplea. En la siguiente dirección \cite{cuda-gpus} se puede comprobar la capacidad de cálculo los distintos modelos:

La capacidad de cálculo mínima para poder utilizar \acrshort{cuda} con al librería tensorflow-gpu es de 3.5. En función de la \acrshort{gpu} que dispongas, ejecute uno de los siguientes comandos para crear un entorno virtual de Anaconda con Python 3.7.0:
