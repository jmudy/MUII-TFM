...
En las siguientes secciones primero se va a detallar las métricas de calidad que se han empleado para validar los datasets de referencia empleados en el entrenamiento de la red neuronal. Después se expondrán los datasets empleados para evaluar los distintos algoritmos que se han propuesto en el capítulo \ref{cha:desarrollo}. Finalmente se expondrán y analizarán los resultados obtenidos al ejecutar los algoritmos sobre los datasets descritos anteriormente.

\section{Entorno experimental}
\label{sec:desarrollo-resultados}

Como ya se vio en la sección \ref{sec:conclu-sota}, \gls{yolov4} es el algoritmo de detección con mayor y precisión y velocidad respecto sus competidores en el Estado del Arte actual. Por otro lado, \gls{deepsort} trabaja de manera muy eficiente con modelos entrenados de \gls{yolov4} de cara al seguimiento de personas y objetos de interés. En esta sección primeramente se va a presentar las métricas de calidad utilizadas para evaluar el dataset de referencia que se eligió en la \textcolor{red}{SECCIÓN X.X} para el entrenamiento de la red neuronal los datasets empleados para ratificar su funcionamiento así como. \textcolor{red}{INTENTAR EXTENDER ESTO PARA QUE EL APARTADO 4.2.1.1.  Intersección sobre la unión (IoU) EMPIECE EN LA SIGUIENTE PAGINA QUE SI NO QUEDA MAL}
...
\subsubsection{Métricas de calidad en MS COCO Dataset}
...
\aviso{Hablar en los datasets de elementos caracterizados que se van a tener en cuenta de cara a la evaluación de los algoritmos, como cambios de luz, resolución, reflejos, lejanía de los objetos, aglomeración de personas con posibilidad de perder el tracking etc \ldots}
\begin{figure}[ht]
  \centering
...
\begin{figure}[ht]
  ...
  \end{subfigure}
  \caption{Imágenes extraídas del datasetAVSSAB 2007 \cite{AVSSAB2007-dataset}.
