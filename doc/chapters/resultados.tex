...
En vista a los resultados, no se han superado las métricas de calidad del dataset \gls{coco} sobre el que se entrenó \gls{yolov4}. Por tanto, se ha determinado continuar el proyecto con el modelo preentrenado de \gls{yolov4} con \gls{coco} como dataset de referencia.
...
Los resultados en los entrenamientos no fueron satisfactorios y se ha continuado el proyecto con el modelo de \gls{yolov4} sobre el dataset \gls{coco}. Se ha llegado a la conclusión de que construir un dataset personalizado que solo detecte un determinado número de clases no es preciso cuando se emplea en escenarios donde hay más objetos detectables. La \gls{cnn} entrenada puede confundir objetos que no son los deseables obteniendo valores bajos de \gls{iou}, lo que deriva a acumular \gls{fp}. Resulta más interesante utilizar un dataset donde se identifiquen una amplia cantidad de clases, como es el ejemplo de \gls{coco}, y posteriormente filtrar las detecciones para solo obtener las que resulten de interés.

Resultaría interesante evaluar las métricas en \gls{yolov4} sobre otros datasets que contengan un gran número de clases, para determinar si se superan las métricas de \gls{coco}, teniendo en cuenta que el principal objetivo de este proyecto es la detección de objetos abandonados.
...
\caption{\label{fig:gba-near-big3-detection-example}Ejemplo 1 detección YOLOv4 Tensorflow}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{img/chapters/resultados/deteccion/avss-easy-detection-example.jpg}
\caption{\label{fig:avss-easy-detection-example}Ejemplo 2 detección YOLOv4 Tensorflow}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{img/chapters/resultados/deteccion/aboda-video5-detection-example.jpg}
\caption{\label{fig:aboda-video5-detection-example}Ejemplo 3 detección YOLOv4 Tensorflow}
\end{figure}
...
\caption{\label{fig:abandoned-avssab2007-example2}Individuo abandona su maleta en en el la zona cercana del andén}
\end{figure}
...
\caption{\label{fig:abandoned-aboda-example1}Individuo abandona su maleta en en el la zona cercana del andén}
\end{figure}
...
\caption{\label{fig:results1}Individuo abandona su maleta en en el la zona cercana del andén}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{img/chapters/resultados/abandono/2.png}
\caption{\label{fig:results2}Individuo abandona su maleta en en el la zona cercana del andén}
\end{figure}
...
\caption{\label{fig:results3}Individuo abandona su maleta en en el la zona cercana del andén}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{img/chapters/resultados/abandono/4.png}
\caption{\label{fig:results4}Individuo abandona su maleta en en el la zona cercana del andén}
\end{figure}
...
\textcolor{red}{Citar el trabajo de David \cite{valdivieso2018}}
